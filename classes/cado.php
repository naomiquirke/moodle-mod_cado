...
class mod_cado_cado {
    ...
    public static function add_instance(stdClass $formdata) {
        ...
        return self::getnewcadorecord($newcadorec); // note can't refer to self as keep public static
    }
    ...
    private function report_course() {
        ...
        $sql = "WITH mod_groups AS ( " .  // Get all the groups that may access each activity module.
            ...
            , chosen_mods AS ( ".
            // Find all the activities that have groups accessing that activity that are in our target grouping, or activities that do not have grouping restrictions.
                ...
                if ($result != 0) {
                    if ( in_array($value, ['forum', 'quiz', 'assign'])) {
                        ...
                        foreach ($ori1 as $value1) {
                            if (!is_numeric($value1['cmid'])) {continue;
                        ...
                        foreach ($oth1 as $value2) {
                            if (!is_numeric($value2['cmid'])) {continue;
                            } //skip the blurb at the beginning of the section; there will be no cmid found for this section
                        ...
                        foreach ($otherinner as $othkey2 => $othval2) {
                            if (!isset($othval2['name']) or $othval2['name'] == 'done') { continue;
    ...
    public static function cleanline($oa) {
        ...
        if ($firstbreak === false) {return false;
